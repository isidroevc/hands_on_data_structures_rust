...
use crate::trees::device_structs::IoTDevice;
...
pub fn test_tree_against_mine() {
    let mut original_tree = BetterDeviceRegistry::new_empty();

    let sequence = generate_random_sequence(12);

    for item in sequence {
        original_tree.add(IoTDevice::new(item, format!("address for: {}", item), format!("format for: {}", item)));
    }

    let  original_walk: RefCell<Vec<u64>> = RefCell::new(vec![]);

    original_tree.walk(|d| {
        original_walk.borrow_mut().push(d.numerical_id);
    });

    assert_eq!(original_walk.borrow().len(), original_walk.borrow().len());
    for i in 0..original_walk.borrow().len() {
        println!("original walk:  {}|| my walk:  {}", original_walk.borrow()[i], 5);
        assert_eq!(original_walk.borrow()[i], 5);

    }

}

fn generate_random_sequence(n: usize) ->  Vec<u64> {
    let mut seq: Vec<u64> = Vec::new();
    let mut rng = rand::thread_rng();

    for i in 0..n {
        seq.push(rng.gen_range(0..99));
    }
